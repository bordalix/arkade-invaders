...
export function startGame(options: InvadersOptions = {}) {
    ...
    class Enemy extends SheetSprite {
        ...
        update(dt: number) {
            ...
            if (this.stepAccumulator >= this.stepDelay) {
                if (
                    this.position.x < this.bounds.w / 2 + 20 &&
                    alienDirection < 0
                ) {
    ...
    class ParticleExplosion {
        ...
        draw() {
            for (let i = this.particles.length - 1; i >= 0; i--) {
                ...
                particle.y +=
                    particle.yunits + particle.gravity * particle.moves;
                particle.life--;
    ...
    function drawStartScreen() {
        ...
        const howTo = isMobile
            ? "Red to shot, gray to move"
            : "X to shoot, arrows to move";
        fillCenteredText(
            howTo,
            CANVAS_WIDTH / 2,
            CANVAS_HEIGHT / 1.66,
