...
import { InsertCoin } from "@bordalix/insert-coin";

const amountSats = import.meta.env.VITE_AMOUNT_SATS || 500;
const arkAddress = import.meta.env.VITE_ARK_ADDRESS || "";
const arkServerUrl =
    import.meta.env.VITE_ARK_SERVER_URL || "https://arkade.computer";
const boltzApiUrl =
    import.meta.env.VITE_BOLTZ_API_URL || "https://api.ark.boltz.exchange";
...
class UI {
    ...
    showQrCode(options: { invoice: string; qrImage: string }): void {
        this.container.innerHTML = `
            <div id="qrCode">
                ${options.qrImage}
